...
var generateRandomAddress = function () {
    var streetNumbers = ['123', '45', '987', '101', '55'];
    var streetNames = ['Main St', 'Oak Ave', 'Pine Ln', 'Maple Rd', 'Cedar Ct'];
    var cities = ['Chennai', 'Coimbatore', 'Bangalore', 'Mumbai', 'Hyderabad'];
    var zipCodes = ['10001', '90210', '75001', '33101', '60601'];
    var randomStreetNum = streetNumbers[Math.floor(Math.random() * streetNumbers.length)];
    var randomStreetName = streetNames[Math.floor(Math.random() * streetNames.length)];
    var randomCity = cities[Math.floor(Math.random() * cities.length)];
    var randomZip = zipCodes[Math.floor(Math.random() * zipCodes.length)];
    return "".concat(randomStreetNum, ", ").concat(randomStreetName, ", ").concat(randomCity, " ").concat(randomZip);
};
// Helper function to generate a random phone number different from an existing one
var generateRandomPhoneNumber = function (existingPhone) {
    var newPhone = '';
    do {
        newPhone = '+91 ' + Array.from({ length: 10 }, function () { return Math.floor(Math.random() * 10); }).join('');
